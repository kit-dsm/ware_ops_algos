...
class HappyChicLoader(DataLoader):
    """
    Loads HappyChic warehouse instances.

    HappyChic instances contain products, orders, and explicit graph structure
    with vertices (intersections, product locations, depot) and arcs.

    Key differences from Foodmart:
    - Multi-dimensional capacity (weight + volume)
    - BoxCanMixOrders = 0 (each box contains one order only)
    - Order box limits (Bo per order)
    ...
    def load(self, filepath: str, use_cache: bool = True) -> BaseWarehouseDomain:
        """
        Load a single HappyChic instance file.
    ...
    def _parse(self, filepath: str) -> Dict[str, Any]:
        """
        Parse a HappyChic format file into structured data.

        The file contains sections for:
        - Header (capacities, dimensions)
        - Products (with locations and multi-dimensional sizes)
        - Orders (with product quantities and box limits)
        - Graph (vertices, arcs, shortest paths)

        Returns:
            Dictionary with keys: header, articles, locations, orders, arcs,
            shortest_paths, vertices_coords, order_box_limits
        """
        ...
        sku_entries = []
        order_entries = []
        order_box_limits = {}  # NEW: Track Bo per order
        arcs = []
        locations = []
        shortest_paths = {}
        vertices_coords = {}
        ...
        # NEW: Handle multi-dimensional capacity
        header["NbDimensionsCapacity"] = int(next_data_line().split()[0])

        # NEW: Read ALL capacity dimensions as list
        capa_line = next_data_line().split()
        header["B_CapaBox"] = [
            int(capa_line[i])
            for i in range(header["NbDimensionsCapacity"])
        ]
        ...
        for _ in range(header["NbProducts"]):
            ...
            # NEW: Read all product dimensions
            dimensions = [
                float(parts[2 + i])
                for i in range(header["NbDimensionsCapacity"])
            ]

            # Use first dimension as "volume" for compatibility
            weight = dimensions[0]
            volume = dimensions[1]
        ...
        for _ in range(header["NbOrders"]):
            ...
            # NEW: Parse Bo (max boxes for this order)
            Bo = int(parts[1])
            order_box_limits[order_number] = Bo
        ...
        while True:
            line = peek_line()
            if line is None or line.startswith("//Location"):  # NEW: Different section marker
                break
            parts = next_line().split()
            if len(parts) >= 3:
        ...
        # NEW: HappyChic uses //Location coordinates LocationName
        skip_to_prefix("//Location coordinates")
        # next_data_line()  # Skip "//Loc x y" header
        ...
        while (line := next_line()) is not None:
            ...
            try:
                ...
                # Determine node type based on label patterns
                if "depot" in label.lower():
                    if idx == departing_depot:
                        node_type = "start_node"
                    elif idx == arrival_depot:
                        node_type = "end_node"
                        x += 1000  # Adjust arrival depot position
                    else:
                        node_type = "depot_node"
                elif "inter" in label.lower():
                    # Intersection nodes (e.g., "aH_aH_inter_0_start")
                    node_type = "intersection"
                else:
                    # Pick locations (e.g., "aC_aC_12", "aE_aE_00")
                    node_type = "pick_node"
        ...
        return {
            ...
            "order_box_limits": order_box_limits,  # NEW
            "arcs": arcs,
    ...
    def _build(self, parsed: Dict[str, Any]) -> BaseWarehouseDomain:
        """
        Build BaseWarehouseDomain from parsed HappyChic data.
        ...
        order_box_limits = parsed["order_box_limits"]  # NEW
        arcs = parsed["arcs"]
        ...
        # print(f"Distance between adjacent picks: {dima.at[(350.0, 4800.0),(350.0, 5340.0)]}")
        ...
        _, predecessors = floyd_warshall(A, directed=True, return_predecessors=True)

        # Convert to DataFrame with coordinate tuples as index/columns
        # dima = pd.DataFrame(fw_distances, index=nodes, columns=nodes)
        # _, predecessors = floyd_warshall(A, directed=True, return_predecessors=True)
        ...
        n_aisles = int(max(x for x, y, _ in vertices_coords.values())) if vertices_coords else 1

        # Find closest node to start (excluding start/end nodes)
        closest_node_to_start = (
            dima[start_node]
            .drop(labels=[start_node, end_node], errors='ignore')
            .idxmin()
        )
        ...
        # Resources
        # NEW: For HappyChic, capacity handling is more complex
        # Since BoxCanMixOrders = 0, we need to track boxes per order
        # For now, store raw capacity values
        capacities = header["B_CapaBox"]  # List: [weight, volume]
