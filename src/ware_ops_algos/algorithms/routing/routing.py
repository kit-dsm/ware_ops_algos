...
@dataclass
class _ProfitableBuild:
    graph: nx.MultiDiGraph
    aisle_ys: dict[int, list[int]]
    position_edges: dict[tuple[int, int], list[tuple]]
...
class ProfitableSprpRouting(RatliffRosenthalRouting):
    ...
    def score(self, input_data):
        raise TypeError(
            "ProfitableSprpRouting requires prepare() and solve_with_scores()"
        )
    ...
    @staticmethod
    def _aisle_relevant_y(build: _ProfitableBuild, j: int) -> list[int]:
        return build.aisle_ys.get(j, [])

    def _build_profitable_state_space(self, build: _ProfitableBuild):
        graph = build.graph
        start_node = (1, ("0", "0", "0C"), "-")
        end_node = (self.n_aisles + 1, ("0", "0", "1C"), "-")
        graph.add_node(start_node, type="start_node", pos=(0, 6))
            ...
            for i, eq_class in enumerate(equivalence_classes):
                for stage in ("-", "+"):
                    if (j == 1 and stage == "-") or (j == self.n_aisles + 1 and stage == "+"):
                        continue
                    graph.add_node((j, eq_class, stage), pos=(2 * (j - 1) + (1.5 if stage == "+" else 0.5), i))
        self._add_aisle_transitions_for(build, 1, [("0", "0", "0C")])
        self._add_profitable_cross_aisle_transitions(build, 1)
        for j in range(2, self.n_aisles + 1):
            self._add_aisle_transitions_for(build, j, equivalence_classes)
            self._add_profitable_cross_aisle_transitions(build, j)

    def _add_profitable_cross_aisle_transitions(self, build: _ProfitableBuild, j: int):
        for prev_eq in equivalence_classes:
            for cross_id, next_eq in table_II.get(prev_eq, {}).items():
                if not next_eq:
                    continue
                action = cross_aisle_mapping[cross_id]
                if not self._is_valid_cross_aisle_transition(j, prev_eq, action):
                    continue
                f = (j, prev_eq, "+")
                t = (j + 1, next_eq, "-")
                if f in build.graph and t in build.graph:
                    build.graph.add_edge(
                        f, t, weight=self.cross_aisle_cost(action), action=action,
                    )
    ...
    def _add_aisle_transitions_for(self, build: _ProfitableBuild, j: int, prev_states):
        is_depot_aisle = j == self.depot[0]
        depot_cost = 2 * self.dist_end if is_depot_aisle else 0.0
        relevant_y = self._aisle_relevant_y(build, j)
        ...
        for prev_eq in prev_states:
            ...
            self._emit_dominated(build, j, prev_eq, ry_set, candidates)

    def _emit_dominated(self, build, j, prev_eq, ry_set, candidates):
        ...
        for next_eq, group in by_target.items():
            ...
            for cost, action_type, action_node, coverage, covered in kept:
                self._emit_edge(build, j, prev_eq, next_eq, action_type, action_node,
                                cost, coverage)

    def _emit_edge(self, build: _ProfitableBuild, j, prev_eq, next_eq, action_type, action_node,
                   cost, coverage):
        f = (j, prev_eq, "-")
        t = (j, next_eq, "+")
        if f not in build.graph or t not in build.graph:
            return
        key = build.graph.add_edge(
            f, t, weight=float(cost), action=action_type,
            action_node=action_node, aisle=j, coverage=coverage,
        )
        ys = build.aisle_ys.get(j, [])
        if not ys or not coverage:
            return
        for (y_min, y_max) in coverage:
            for y in ys:
                if y_min <= y <= y_max:
                    build.position_edges[(j, y)].append((f, t, key))
    ...
    def prepare(self, order_positions, demands, capacity, *,
        ...
        for positions in order_positions:
            for (aisle, y) in positions:
                aisle_ys.setdefault(aisle, set()).add(y)
        build = _ProfitableBuild(
            graph=nx.MultiDiGraph(),
            aisle_ys={a: sorted(ys) for a, ys in aisle_ys.items()},
            position_edges=defaultdict(list),
        )
        self._build_profitable_state_space(build)
        start_node = (1, ("0", "0", "0C"), "-")
        end_node = (self.n_aisles + 1, ("0", "0", "1C"), "-")
        mdl = gp.Model("ProfitableSPRP")
        mdl.setParam("OutputFlag", 0)
        mdl.setParam("Threads", 1)
        edge_list = list(build.graph.edges(keys=True, data=True))
        x = {}
        for u, v, k, data in edge_list:
            x[(u, v, k)] = mdl.addVar(
                vtype=GRB.BINARY,
        ...
        for node in build.graph.nodes():
            out_expr = gp.quicksum(
                x[(u, v, k)]
                for u, v, k in build.graph.out_edges(node, keys=True)
            )
            in_expr = gp.quicksum(
                x[(u, v, k)]
                for u, v, k in build.graph.in_edges(node, keys=True)
            )
        ...
        for i, positions in enumerate(order_positions):
            for (aisle, yv) in positions:
                edges = build.position_edges.get((aisle, yv), [])
                if not edges:
                    mdl.addConstr(y[i] == 0)
                    continue
        ...
        if not allow_empty and n_orders > 0:
            feasible = [
                ...
                if all(build.position_edges.get((a, yv)) for (a, yv) in positions)
            ]
            if feasible:
        ...
        return {
            ...
            "graph": build.graph,
            "position_edges": build.position_edges,
        }
        ...
        for prev_eq in prev_states:
            ...
            if use_subnet:
                self._build_rg_subnetwork(
                    build, j, prev_eq, gap_target, relevant_y,
                    depot_cost, M, include_0, include_M)

    def _build_rg_subnetwork(self, build: _ProfitableBuild, j, prev_eq, gap_target, relevant_y,
                              depot_cost, M, include_0, include_M):
        ...
        if from_node not in build.graph or to_node not in build.graph:
            return
        ...
        for w, (neg_h, neg_i) in zip(aux_nodes, negs):
            for h in h_set:
                ...
                for (y_min, y_max) in coverage:
                    for y in ys:
                        if y_min <= y <= y_max:
                            build.position_edges[(j, y)].append(
                                (from_node, w, key))
